<template>
  <input :value="formatarValor(modelValue)"
    @input="emits('update:modelValue',  removerMascara($event.target.value))" :maxlength="16" @keypress="keyPress" />
</template>
...
function getInputMask() {
  if (removerMascara(props.modelValue).length > 10)
    return "(##) # ####-####";
  else return "(##) ####-####";
}
...
function formatarValor(value) {
  if (value) {
    return formataTelefone(value, getInputMask());
  } else {
    return "";
  }
}
...
function formataTelefone(value, mask) {
